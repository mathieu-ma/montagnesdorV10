...
public class ColorRectangleCanvas extends Canvas
{
	...
	ColorRectangleCanvas(int width, int height)
    {
        ...
        do
        {
            int j = 1;
            do
                colorMap[i + (height - j) * width] = Color.HSBtoRGB((float)i / width, (float)j / height, 0.75F);
            while(++j <= height);
        } while(++i < width);
        rectangleColor = createImage(new MemoryImageSource(width, height, colorMap, 0, width));
    }
