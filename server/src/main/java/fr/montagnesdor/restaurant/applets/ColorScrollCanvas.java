...
public class ColorScrollCanvas extends Canvas
{
    ...
    {
        int ai[] = { 0, gap, 0 };
        int ai1[] = { mouseY - gap, mouseY, mouseY + gap };
    ...
    public void update(Graphics g)
    {
        ...
        g.clearRect(getSize().width - gap, 0, getSize().width, getSize().height);
        paint(g);
    }
    ...
    public void refresh(Color color)
    {
        ...
        do
        {
            int j = 1;
            do
                colorMap[i + j * width] = 0xff000000 | (j * 2 * (color.getRed() - 255)) / height + 255 << 16 | (j * 2 * (color.getGreen() - 255)) / height + 255 << 8 | (j * 2 * (color.getBlue() - 255)) / height + 255;
            while (++j <= (height / 2));
        } while (++i < width);
        i = 0;
        do
        {
            int k = (height / 2);
            do
                colorMap[i + k * width] = 0xff000000 | (color.getRed() * 2 * (height - k)) / height << 16 | (color.getGreen() * 2 * (height - k)) / height << 8 | (color.getBlue() * 2 * (height - k)) / height;
            while (++k < height);
        } while (++i < width);
        scrollColor = createImage(new MemoryImageSource(width, height, colorMap, 0, width));
        repaint();
    }
