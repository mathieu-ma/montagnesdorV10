...
public final class ColorPickerApplet extends Applet implements MouseListener, MouseMotionListener, KeyListener
{
    ...
    private String startJavascriptFunction = "initPage";
        
    private void processEvent(MouseEvent mouseevent)
    {
        if (mouseevent.getComponent() instanceof ColorRectangleCanvas)
        {
            Color color = colorRectangleCanvas.getColorAt(mouseevent.getX(), mouseevent.getY());
            if (color != null)
            {
        ...
        {
            Color color = colorScrollCanvas.getColorAt(mouseevent.getX(), mouseevent.getY());
            updateText(color);
...
//        System.out.println("processEvent : " + hexaColorTextField.getText());
    ...
    private void retrieveParameters()
    {
        ...
        if (getParameter("startJavascriptFunction") != null)
    ...
    public void init()
    {
        ...
        defaultColor = new Color(Integer.parseInt(defaultRGBColor.substring(0, 2), 16), Integer.parseInt(defaultRGBColor.substring(2, 4), 16), Integer.parseInt(defaultRGBColor.substring(4, 6), 16));

        setLayout(new BorderLayout());
        setBackground(Color.white);

        Panel centerPanel = new Panel(new FlowLayout(FlowLayout.CENTER, getWidth() * 1 / 100, getHeight() * 1 / 100));
        colorRectangleCanvas = new ColorRectangleCanvas(getWidth() * 83 / 100, getHeight() * 83 / 100);
        ...
        colorScrollCanvas = new ColorScrollCanvas(getWidth() * 7 / 100, getHeight() * 83 / 100, defaultColor);
        ...
        Panel southPanel = new Panel(new FlowLayout(FlowLayout.CENTER, getWidth() * 1 / 100, getHeight() * 1 / 100));
        int fontSize = (int) Math.round(3 * 12.0 * getHeight() * 10 / 100 / 72.0);
        ...
        hexaColorTextField = new TextField(defaultRGBColor, defaultRGBColor.length());
        hexaColorTextField.setName("hexaColorTextField");
        hexaColorTextField.setSize(getWidth() * 20 / 100, getHeight() * 10 / 100);
    ...
    public void keyReleased(KeyEvent keyEvent)
    {
        if (keyEvent.getComponent().getName().equals("hexaColorTextField"))
        {
            ...
            Color color = new Color(Integer.parseInt(RGBColor.substring(0, 2), 16), Integer.parseInt(RGBColor.substring(2, 4), 16), Integer.parseInt(RGBColor.substring(4, 6), 16));
            colorScrollCanvas.refresh(color);
            colorScrollCanvas.repaint();
            updateText(color);
            hexaColorTextField.setCaretPosition(previousCaret);
            colorCanvas.setBackground(color);
//            System.out.println("keyReleased : " + hexaColorTextField.getText());
        }
    }
    ...
    public void start()
    {
        ...
        win.eval(startJavascriptFunction+"();");
        }
        });
